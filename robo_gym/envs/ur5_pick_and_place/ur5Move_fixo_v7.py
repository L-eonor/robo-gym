...
class UR5RobotiqEnv(gym.Env):
    ...
    def __init__(self, rs_address=None, max_episode_steps=600, robotiq=85, **kwargs):
    ...
    def _get_destination_pose(self):
        ...
        #pose=[ 0.50, 0.15, cube_height/2]
        pose =[ 0.45, 0.13, cube_height/2]
    ...
    def _get_observation_space_with_cubes(self, number_of_objs):
        ...
        abs_max_angle=np.pi + angle_tolerance 
        max_gripper_pose=[ abs_max_gripper_pose,  abs_max_gripper_pose,  0.41 ]
        min_gripper_pose=[-0.1, -abs_max_gripper_pose, -0.1 ]

        #gripper_to_obj_dist
        max_obj_to_target=[ 2* abs_max_gripper_pose, 2* abs_max_gripper_pose, 2* abs_max_gripper_pose]
        min_obj_to_target=[ 1*-abs_max_gripper_pose, 2*-abs_max_gripper_pose, 2*-abs_max_gripper_pose]

        max_1_obj_pos=[ abs_max_gripper_pose,  abs_max_gripper_pose, np.inf]
        min_1_obj_pos=[0, -abs_max_gripper_pose,      0]
        ...
        max_observation = np.concatenate(( max_gripper_pose, max_obj_to_target, max_n_obj_pos, max_gripper_state, max_grasping))
        min_observation = np.concatenate(( min_gripper_pose, min_obj_to_target, min_n_obj_pos, min_gripper_state, min_grasping))
    ...
    def _is_gripper_closed(self):
        ...
        return False if self.state.state["ur_j_pos_norm"].get_finger_joints_value() < -0.5 else True

    def _is_grasping(self):
        #open: x<0.01, close:x>=0.01
        is_gripper_closed=self._is_gripper_closed()
    ...
    def _get_obs(self):
        ...
        is_closed= 0 if not self._is_gripper_closed() else 1
        is_grasping= 1 if self._is_grasping() else 0
        obs=np.concatenate((obs, [is_closed], [is_grasping]))
        return obs
...
class env_state():
    ...
    def get_obs(self):
        ...
        observation  = np.concatenate([self.state["gripper_pose"], self.state["destination_pose"][:3], self.state["cubes_pose"][0, 1:4].reshape(-1)]).reshape(-1) #só um cubo
...
class GripperMoveFixUR5(UR5RobotiqEnv):
    ...
    def reward_done_info(self, achieved_goal, desired_goal):
        ...
        gripper_pose = self.state.state["gripper_pose"]
        cube_real_pose   = np.array(self.state.state["cubes_pose"][0, 1:4].reshape(-1) )
        target_pose=np.array(self.destination_pose)
        #euclidean norm
        gripper_to_cube = np.absolute(self._distance_to_goal(gripper_pose, cube_real_pose)).reshape(-1)
        cube_to_goal = np.absolute(self._distance_to_goal(target_pose, cube_real_pose)).reshape(-1)

        # Reward base
        if self.has_picked:
            reward = -0.5 * cube_to_goal[0]
        else:
        ...
        #reward for moving
        if self.has_picked and (np.absolute(cube_to_goal[0])<=self.move_threshold):
            corrected_reward=np.array(18.0, dtype='float32')
            info = {
            'is_success': True,
            'final_status': 'Moved',
            }
        ...
        #reward for grasping
        elif self._is_grasping() and not self.has_picked:
...
class GripperMoveFixUR5Sim_v7(GripperMoveFixUR5, Simulation):
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, **kwargs):
        ...
        GripperMoveFixUR5.__init__(self, rs_address=self.robot_server_ip, max_episode_steps=1000, robotiq=85, **kwargs)